...
export default function Dither565({
  setFilters,
  filters,
}: {
  ...
  return (
    <label className="flex items-center mb-5">
      <p className="capitalize">Dither565</p>
      <input
        name="dither565"
        type="number"
        ...
        value={filters.dither565 === -1 ? "" : filters.dither565}
        onChange={(e) => {
          if (!e.target.value) {
          ...
          if (
            ...
            return setFilters((prev) => ({
              ...prev,
              dither565: Number(e.target.value),
            }));
          }
        }}
      />
      <IconCloseCircle
        ...
        className={filters.dither565 === -1 ? "opacity-0" : ""}
      />
    </label>
  );
}
